...
async fn restyle_clip(
    ...
    if let Some(dir) = manifest.output_dir.clone() {
        let dest = std::path::Path::new(&dir).join(&clip.filename);
        tokio::fs::copy(&final_path, &dest).await.ok();
        tokio::fs::copy(final_path.with_extension("srt"), dest.with_extension("srt"))
            .await
            .ok();
    }
